...
__Z_INLINE void extractHDPath(uint32_t rx, uint32_t offset) {
  ...
  if (pathLength != HDPATH_LEN_DEFAULT ||
      (rx - offset) != sizeof(uint32_t) * pathLength) {
  ...
  if (hdPath[0] == HDPATH_0_DEFAULT && hdPath[1] == HDPATH_1_DEFAULT) {
    mainnet = true;
    encoding = addr_mainnet_transparent;
  }

  if (hdPath[0] == HDPATH_0_DEFAULT && hdPath[1] == HDPATH_1_TESTNET) {
    testnet = true;
    encoding = addr_testnet_transparent;
  }
...
__Z_INLINE void handleSignTransaction(volatile uint32_t *flags,
...
#if defined(APP_TESTING)
void handleTest(__Z_UNUSED volatile uint32_t *flags,
                __Z_UNUSED volatile uint32_t *tx, __Z_UNUSED uint32_t rx) {
  THROW(APDU_CODE_OK);
}
#endif
