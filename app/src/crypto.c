...
static zxerr_t crypto_extractPublicKey_ed25519(uint8_t *pubKey,
                                               uint16_t pubKeyLen) {
  ...
  CATCH_CXERROR(os_derive_bip32_with_seed_no_throw(
      HDW_ED25519_SLIP10, CX_CURVE_Ed25519, hdPath, HDPATH_LEN_DEFAULT,
      privateKeyData, NULL, NULL, 0));

  CATCH_CXERROR(cx_ecfp_init_private_key_no_throw(
      CX_CURVE_Ed25519, privateKeyData, SK_LEN_25519, &cx_privateKey));
  CATCH_CXERROR(cx_ecfp_init_public_key_no_throw(CX_CURVE_Ed25519, NULL, 0,
                                                 &cx_publicKey));
  CATCH_CXERROR(cx_ecfp_generate_pair_no_throw(CX_CURVE_Ed25519, &cx_publicKey,
                                               &cx_privateKey, 1));
...
static zxerr_t crypto_sign_ed25519(uint8_t *output, uint16_t outputLen,
                                   const uint8_t *message,
                                   uint16_t messageLen) {
  if (output == NULL || message == NULL || outputLen < ED25519_SIGNATURE_SIZE ||
      messageLen == 0) {
  ...
  CATCH_CXERROR(cx_ecfp_init_private_key_no_throw(
      CX_CURVE_Ed25519, privateKeyData, SK_LEN_25519, &cx_privateKey));
  CATCH_CXERROR(cx_eddsa_sign_no_throw(&cx_privateKey, CX_SHA512, message,
                                       messageLen, output, outputLen));
  error = zxerr_ok;
...
zxerr_t crypto_fillAddress_ed25519(uint8_t *buffer, uint16_t bufferLen,
  ...
  CHECK_ZXERR(
      crypto_extractPublicKey_ed25519(answer->publicKey + 1, PK_LEN_25519));

  outLen = crypto_encodePubkey_ed25519(answer->address, sizeof(answer->address),
                                       encoding, answer->publicKey + 1);
...
zxerr_t crypto_fillAddress(signing_key_type_e addressKind, uint8_t *buffer,
                           uint16_t bufferLen, uint16_t *addrResponseLen) {
...
static zxerr_t crypto_hashFeeHeader(const header_t *header, uint8_t *output,
                                    uint32_t outputLen) {
...
static zxerr_t crypto_hashRawHeader(const header_t *header, uint8_t *output,
                                    uint32_t outputLen) {
  if (header == NULL || output == NULL || outputLen < CX_SHA256_SIZE) {
    return zxerr_invalid_crypto_settings;
  }
  ...
  CHECK_CX_OK(cx_sha256_update(&sha256, &header_discriminant,
                               sizeof(header_discriminant)));
  CHECK_CX_OK(cx_sha256_final(&sha256, output));
  return zxerr_ok;
}

zxerr_t crypto_hashSigSection(const signature_section_t *signature_section,
                              const uint8_t *prefix, uint32_t prefixLen,
                              uint8_t *output, uint32_t outputLen) {
  if (signature_section == NULL || output == NULL ||
      outputLen < CX_SHA256_SIZE) {
    return zxerr_invalid_crypto_settings;
  }
  ...
  CHECK_CX_OK(cx_sha256_update(
      &sha256, (uint8_t *)&signature_section->hashes.hashesLen, 4));
  CHECK_CX_OK(cx_sha256_update(&sha256, signature_section->hashes.hashes.ptr,
                               HASH_LEN * signature_section->hashes.hashesLen));
  CHECK_CX_OK(cx_sha256_update(
      &sha256, (uint8_t *)&signature_section->signerDiscriminant, 1));

  switch (signature_section->signerDiscriminant) {
    case PubKeys: {
      CHECK_CX_OK(cx_sha256_update(
          &sha256, (uint8_t *)&signature_section->pubKeysLen, 4));
      uint32_t pos = 0;
      for (uint32_t i = 0; i < signature_section->pubKeysLen; i++) {
        ...
        const uint8_t pubKeySize =
            tag == key_ed25519 ? PK_LEN_25519 : COMPRESSED_SECP256K1_PK_LEN;
    ...
    case Address:
      CHECK_CX_OK(cx_sha256_update(&sha256, signature_section->address.ptr,
                                   signature_section->address.len));
      break;
  ...
  CHECK_CX_OK(cx_sha256_update(
      &sha256, (const uint8_t *)&signature_section->signaturesLen, 4));
  uint32_t pos = 0;
  for (uint32_t i = 0; i < signature_section->signaturesLen; i++) {
    ...
    const uint8_t signatureSize =
        tag == key_ed25519 ? ED25519_SIGNATURE_SIZE : SIG_SECP256K1_LEN;
    // Skip the signature proper
    pos += signatureSize;
  }
  if (pos > 0) {
    CHECK_CX_OK(cx_sha256_update(
        &sha256, signature_section->indexedSignatures.ptr, pos));
  }
  CHECK_CX_OK(cx_sha256_final(&sha256, output));
  return zxerr_ok;
}

static zxerr_t crypto_addTxnHashes(const parser_tx_t *txObj,
  ...
  switch (txObj->typeTx) {
    case InitAccount:
      MEMCPY(hashes->hashes.ptr + hashes->hashesLen * HASH_LEN,
             txObj->initAccount.vp_type_sechash.ptr, HASH_LEN);
      hashes->indices.ptr[hashes->hashesLen] =
          txObj->initAccount.vp_type_secidx;
      hashes->hashesLen++;
      break;

    case UpdateVP:
      MEMCPY(hashes->hashes.ptr + hashes->hashesLen * HASH_LEN,
             txObj->updateVp.vp_type_sechash.ptr, HASH_LEN);
    ...
    case InitProposal:
      MEMCPY(hashes->hashes.ptr + hashes->hashesLen * HASH_LEN,
             txObj->initProposal.content_sechash.ptr, HASH_LEN);
      hashes->indices.ptr[hashes->hashesLen] =
          txObj->initProposal.content_secidx;
      hashes->hashesLen++;
      if (txObj->initProposal.has_proposal_code) {
        MEMCPY(hashes->hashes.ptr + hashes->hashesLen * HASH_LEN,
               txObj->initProposal.proposal_code_sechash.ptr, HASH_LEN);
        hashes->indices.ptr[hashes->hashesLen] =
            txObj->initProposal.proposal_code_secidx;
        hashes->hashesLen++;
      }
      break;
...
zxerr_t crypto_sign(const parser_tx_t *txObj, uint8_t *output,
                    uint16_t outputLen) {
  const uint16_t minimumBufferSize = PK_LEN_25519_PLUS_TAG + 2 * SALT_LEN +
                                     2 * SIG_LEN_25519_PLUS_TAG + 2 + 10;
  ...
  // Hashes: code, data, (initAcc | initVali | updateVP = 1  /  initProp = 2),
  // raw_signature, header ---> MaxHashes = 6
  uint8_t hashes_buffer[MAX_SIGNATURE_HASHES * HASH_LEN] = {0};
  uint8_t indices_buffer[MAX_SIGNATURE_HASHES] = {0};
  concatenated_hashes_t section_hashes = {.hashes.ptr = hashes_buffer,
  ...
  CHECK_ZXERR(
      crypto_hashRawHeader(&txObj->transaction.header, rawHeaderHash, HASH_LEN))
  section_hashes.hashesLen = 1;

  char hexString[100] = {0};
  array_to_hexstr(hexString, sizeof(hexString), section_hashes.hashes.ptr,
                  HASH_LEN);
  ...
  uint8_t *raw_signature_hash =
      section_hashes.hashes.ptr + (section_hashes.hashesLen * HASH_LEN);
  CHECK_ZXERR(crypto_hashSigSection(&signature_section, NULL, 0,
                                    raw_signature_hash, HASH_LEN))

  // Sign over the hash of the unsigned signature section
  uint8_t *raw = salt_buffer + SALT_LEN;
  CHECK_ZXERR(crypto_sign_ed25519(raw + 1, ED25519_SIGNATURE_SIZE,
                                  raw_signature_hash, HASH_LEN))

  uint8_t raw_indices_len = section_hashes.hashesLen;
  uint8_t raw_indices_buffer[MAX_SIGNATURE_HASHES] = {0};
  MEMCPY(raw_indices_buffer, section_hashes.indices.ptr,
  ...
  // Use previous byte from salt that is always 0x00 but we're not sending an
  // extra byte in the response since raw points to output buffer
  ...
  CHECK_ZXERR(crypto_hashSigSection(&signature_section, &sig_sec_prefix, 1,
                                    raw_signature_hash, HASH_LEN))
  section_hashes.indices.ptr[section_hashes.hashesLen] =
      txObj->transaction.sections.sectionLen + 1 + 0 /*signature_raw*/;
  section_hashes.hashesLen++;
  ...
  for (uint32_t i = 0; i < txObj->transaction.sections.signaturesLen; i++) {
    const signature_section_t *prev_sig =
        &txObj->transaction.sections.signatures[i];
    unsigned int j;
    ...
    for (j = 0; j < prev_sig->hashes.hashesLen; j++) {
      ...
      for (k = 0; k < signature_section.hashes.hashesLen; k++) {
        if (!memcmp(prev_sig->hashes.hashes.ptr,
                    signature_section.hashes.hashes.ptr, HASH_LEN)) {
          break;
        }
      }
    ...
    uint8_t *prev_sig_hash =
        section_hashes.hashes.ptr + (section_hashes.hashesLen * HASH_LEN);
    CHECK_ZXERR(crypto_hashSigSection(prev_sig, &sig_sec_prefix, 1,
                                      prev_sig_hash, HASH_LEN))
  ...
  CHECK_ZXERR(
      crypto_hashFeeHeader(&txObj->transaction.header, header_hash, HASH_LEN))
  ...
  CHECK_ZXERR(crypto_hashSigSection(&signature_section, NULL, 0,
                                    wrapper_sig_hash, sizeof(wrapper_sig_hash)))

  // Sign over the hash of the unsigned signature section
  uint8_t *wrapper = raw + SALT_LEN + SIG_LEN_25519_PLUS_TAG;
  CHECK_ZXERR(crypto_sign_ed25519(wrapper + 1, ED25519_SIGNATURE_SIZE,
                                  wrapper_sig_hash, sizeof(wrapper_sig_hash)))
  ...
  for (uint8_t i = 0; i < section_hashes.hashesLen; i++) {
    char hexString[100] = {0};
    array_to_hexstr(hexString, sizeof(hexString),
                    section_hashes.hashes.ptr + (HASH_LEN * i), HASH_LEN);
