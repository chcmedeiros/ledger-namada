...
address_kind_e encoding = addr_mainnet_transparent;

static zxerr_t crypto_publicKeyHash_ed25519(uint8_t *publicKeyHash,
                                            const uint8_t *pubkey) {
  if (publicKeyHash == NULL || pubkey == NULL) {
    return zxerr_no_data;
  }

  // Step 1.  First borsh serialize pubkey (this prepends a 0 to the bytes of
  // pubkey);
...
#if defined(TARGET_NANOS) || defined(TARGET_NANOS2) || \
    defined(TARGET_NANOX) || defined(TARGET_STAX)
  cx_hash_sha256((const uint8_t *)borshEncodedPubKey, PK_LEN_25519 + 1, pkh,
                 CX_SHA256_SIZE);
#else
  picohash_ctx_t ctx;
  ...
  array_to_hexstr_uppercase(hexPubKeyHash, 2 * CX_SHA256_SIZE + 1, pkh,
                            CX_SHA256_SIZE);

  // Prepend implicit address prefix
  publicKeyHash[0] = 0;

  // Step 4. The Public Key Hash consists of the first 40 characters of the hex
  // encoding. ---> UPPERCASE
...
uint8_t crypto_encodePubkey_ed25519(uint8_t *buffer, uint16_t bufferLen,
  ...
  zxerr_t err = bech32EncodeFromBytes(
      addr_out, sizeof(addr_out),
      encode == addr_mainnet_transparent ? MAINNET_TRANSPARENT_HRP
                                         : TESTNET_TRANSPARENT_HRP,
...
zxerr_t crypto_sha256(const uint8_t *input, uint16_t inputLen, uint8_t *output,
                      uint16_t outputLen) {
...
zxerr_t crypto_hashExtraDataSection(const section_t *extraData, uint8_t *output,
                                    uint32_t outputLen) {
  ...
  CHECK_CX_OK(cx_sha256_update(&sha256, (uint8_t *)&extraDataTagLen,
                               has_tag * sizeof(extraDataTagLen)));
  CHECK_CX_OK(
      cx_sha256_update(&sha256, extraData->tag.ptr, has_tag * extraDataTagLen));
  ...
  picohash_update(&sha256, (uint8_t *)&extraDataTagLen,
                  has_tag * sizeof(extraDataTagLen));
...
zxerr_t crypto_hashDataSection(const section_t *data, uint8_t *output,
                               uint32_t outputLen) {
  ...
  CHECK_CX_OK(cx_sha256_update(&sha256, (uint8_t *)&dataBytesLen,
                               sizeof(dataBytesLen)));
...
zxerr_t crypto_hashCodeSection(const section_t *code, uint8_t *output,
                               uint32_t outputLen) {
  ...
  CHECK_CX_OK(cx_sha256_update(&sha256, &has_tag, 1));
  CHECK_CX_OK(cx_sha256_update(&sha256, (uint8_t *)&codeTagLen,
                               has_tag * sizeof(codeTagLen)));
  ...
  uint8_t has_tag = (code->tag.ptr == NULL) ? 0 : 1;
  picohash_update(&sha256, &has_tag, 1);
  picohash_update(&sha256, (uint8_t *)&codeTagLen,
                  has_tag * sizeof(codeTagLen));
...
zxerr_t crypto_serializeData(const uint64_t dataSize, uint8_t *buffer,
                             uint16_t bufferLen, uint8_t *dataInfoSize) {
  if (bufferLen < 11) {
    return zxerr_buffer_too_small;
  }
  ...
  buffer[0] = 18;
  CHECK_ZXERR(
      encodeLEB128(dataSize, buffer + 1, MAX_LEB128_OUTPUT, dataInfoSize))
